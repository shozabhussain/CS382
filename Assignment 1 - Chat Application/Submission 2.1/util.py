...
def list_to_string(msg_str):

    ''' convert a list into string'''

    str1 = ""
    str1 = msg_str[0]
    for item in range(1, len(msg_str)):
        str1 = str1 + " " + msg_str[item]

    return str1

def str_to_chunks(msg_str, chunk_size):

    '''takes msg from make_msg and divide it into chunks'''

    chunk_list = []

    for item in range(0,len(msg_str),chunk_size):
        chunk_list.append(msg_str[item:item+chunk_size])
